...
export async function addCommentToPR(fileName: string, comment: string, httpsAgent: Agent) {
  ...
  const prUrl = `${tl.getVariable('SYSTEM.TEAMFOUNDATIONCOLLECTIONURI')}${tl.getVariable(
    'SYSTEM.TEAMPROJECTID',
  )}/_apis/git/repositories/${tl.getVariable(
    'Build.Repository.Name',
  )}/pullRequests/${tl.getVariable('System.PullRequest.PullRequestId')}/threads?api-version=5.1`;
...
export async function deleteExistingComments(httpsAgent: Agent) {
  ...
  const threadsUrl = `${tl.getVariable('SYSTEM.TEAMFOUNDATIONCOLLECTIONURI')}${tl.getVariable(
    'SYSTEM.TEAMPROJECTID',
  )}/_apis/git/repositories/${tl.getVariable(
    'Build.Repository.Name',
  )}/pullRequests/${tl.getVariable('System.PullRequest.PullRequestId')}/threads?api-version=5.1`;
  ...
  for (const thread of threadsWithContext as any[]) {
    const commentsUrl = `${tl.getVariable('SYSTEM.TEAMFOUNDATIONCOLLECTIONURI')}${tl.getVariable(
      'SYSTEM.TEAMPROJECTID',
    )}/_apis/git/repositories/${tl.getVariable(
      'Build.Repository.Name',
    )}/pullRequests/${tl.getVariable('System.PullRequest.PullRequestId')}/threads/${
      thread.id
    ...
    for (const comment of comments.value.filter(
      ...
      )}${tl.getVariable('SYSTEM.TEAMPROJECTID')}/_apis/git/repositories/${tl.getVariable(
        'Build.Repository.Name',
      )}/pullRequests/${tl.getVariable('System.PullRequest.PullRequestId')}/threads/${
        thread.id
